    ...
    <div>
      ...
      >
        <div className="flex justify-between items-center bg-jili-bgBlack px-4  py-1 relative ">
          ...
          {/* array button */}
          <div
  className={`fixed left-0 transform transition-transform duration-300 bg-black  w-20 h-14 flex items-center justify-center ${
    isSidebarOpen ? "translate-x-60" : "translate-x-60"
  }`}
>
  <button
    className="bg-backgroundV2Color rounded-full text-black py-1 px-3 shadow-lg"
    onClick={toggleSidebar}
  >
    <FaChevronRight size={16} />
        ...
        <ul className="space-y-4 overflow-y-auto scrollbar-hide h-screen">
          {icons?.map((item) => (
            <li key={item.key}>
              <div
